...
// Owner numbers
const ownerNumber = ["923554583937"];
...
function loadLocalFiles() {
  ...
  if (fs.existsSync('./lib/subhan.html')) {
    console.log("✅ subhan.html found in lib folder");
  } else {
    console.log("⚠️ subhan.html not found in lib folder");
  }
...
app.get('/lib/subhan.html', (req, res) => {
  res.redirect('/');
});
...
async function getProfilePicture(sock, jid) {
  try {
    ...
    return 'https://pro-hostify.free.nf//uploads/e95bec98_file_0000000014cc71fabc4b2e31bdf297e8.png'; // Default image
  }
}
...
async function connectToWA() {
  ...
  console.log(`🤖 SUBHAN-MD Connecting with prefix: "${prefix}"`);
  ...
  sock.ev.on('connection.update', (update) => {
    ...
    if (connection === 'close') {
      ...
      console.log("✅ SUBHAN-MD Bot connected to WhatsApp!");
      ...
      const aliveMsg = `*╭──────────────●●►*\n> *SUBHAN-MD CONNECTED SUCCESSFULLY*\n\n> *Type ${prefix}menu to view commands*  \n\n*╭⊱✫ SUBHAN MD ✫⊱╮*\n*│✫📂 Bot Name: ${botConfig.BOT_NAME}*\n*│✫🛡️ Owner: ${config.OWNER_NAME}*\n*│✫♻️ Prefix: ${prefix}*\n*│✫🌍 Mode: ${config.MODE}*\n*│✫⏰ Uptime: ${runtime(process.uptime())}*\n*╰──────────────●●►*\n\n> Enjoy Using SUBHAN MD`;
      
      // Image URL for connection message
      const imageUrl = 'https://pro-hostify.free.nf//uploads/e95bec98_file_0000000014cc71fabc4b2e31bdf297e8.png';
      
      try {
        // Send to owner with image
        sock.sendMessage(ownerNumber[0] + '923554583937@s.whatsapp.net', {
          image: { url: imageUrl },
          caption: aliveMsg
        }).catch(() => {
      ...
      for (const participant of participants) {
        ...
        if (action === 'add') {
          ...
          if (settings.goodbye) {
            try {
              // Get user's profile picture
              const ppUrl = await getProfilePicture(sock, participant).catch(() => 'https://pro-hostify.free.nf//uploads/e95bec98_file_0000000014cc71fabc4b2e31bdf297e8.png');
